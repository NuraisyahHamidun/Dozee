<x-app-layout>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="rounded-3xl bg-indigo-600 overflow-hidden relative group shadow-2xl shadow-indigo-200">
                        ...
                        <div class="relative p-8 flex flex-col items-center text-center">
                            @if($salesman->profile_picture)
                                <img src="{{ asset('storage/' . $salesman->profile_picture) }}" class="w-36 h-36 rounded-3xl object-cover border border-white/30 shadow-xl mb-6 group-hover:scale-105 transition-transform duration-500" alt="Avatar">
                            @else
                            ...
                            @endif
                            <h3 class="text-xl font-black text-white tracking-tight mb-6">{{ $salesman->name }}</h3>
                    ...
                    <div class="premium-card bg-white dark:bg-slate-800 p-6 space-y-4">
                        ...
                        <div class="flex items-center gap-4 text-slate-600 dark:text-slate-400">
                            ...
                            <span class="text-xs font-bold">{{ $salesman->username }}</span>
                        </div>
                        <div class="flex items-center gap-4 text-slate-600 dark:text-slate-400">
                            ...
                            <span class="text-xs font-bold">{{ $salesman->email }}</span>
                        </div>
                        <div class="flex items-center gap-4 text-slate-600 dark:text-slate-400">
                            <svg class="w-5 h-5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                            <span class="text-xs font-bold">{{ $salesman->phone_number ?? 'Tiada nombor telefon' }}</span>
                        </div>
                        <div class="flex items-center gap-4 text-slate-600 dark:text-slate-400">
                            <svg class="w-5 h-5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                            <span class="text-xs font-bold">{{ $salesman->address ?? 'No address registered' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="premium-card bg-white dark:bg-slate-800 p-8 shadow-xl border-none">
                        <form method="POST" action="{{ route('salesman.profile.update') }}" enctype="multipart/form-data" class="space-y-8">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <input id="username" name="username" type="text" readonly
                                           ...
                                           value="{{ old('username', $salesman->username) }}" />
                                ...
                                <div class="flex items-center gap-6">
                                    @if($salesman->profile_picture)
                                        <img id="profile-preview" src="{{ asset('storage/' . $salesman->profile_picture) }}" class="w-16 h-16 rounded-2xl object-cover border border-slate-200 dark:border-slate-700 shadow-md" alt="Preview">
                                    @else
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input id="email" name="email" type="email" readonly
                                               ...
                                               value="{{ old('email', $salesman->email) }}" required />
                                    ...
                                    <div>
                                        ...
                                        <input id="phone_number" name="phone_number" type="text" required
                                               ...
                                               value="{{ old('phone_number', $salesman->phone_number) }}" placeholder="e.g. 012-3456789 or 012-34567890" />
                    ...
                    <div class="premium-card p-10 bg-slate-50 dark:bg-slate-900/50 border-2 border-dashed border-red-100 dark:border-red-900/20">
                        <div class="flex flex-col md:flex-row items-center gap-8">
                            ...
                            <form method="POST" action="{{ route('salesman.profile.destroy') }}" id="salesman-delete-form">
                                @csrf
                                @method('DELETE')
                                <button type="button" id="salesman-delete-btn" class="w-full md:w-auto bg-red-500 text-white font-black text-[10px] uppercase tracking-[0.2em] px-8 py-4 rounded-2xl hover:bg-red-600 shadow-xl shadow-red-100 transition-all italic">
                                    Revoke Account
                                </button>
                            </form>
                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const deleteStaffBtn = document.getElementById('salesman-delete-btn');
            if (deleteStaffBtn) {
                deleteStaffBtn.addEventListener('click', function () {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            document.getElementById('salesman-delete-form').submit();
                        }
                    });
                });
            }
