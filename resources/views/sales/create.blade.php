<x-app-layout>
    ...
    {{-- Bundle items URL for JS --}}
    <script>
        window.bundleItemsBaseUrl = "{{ url('/promotions') }}";
        window.csrfToken = "{{ csrf_token() }}";
        window.promoData = @json($promotions->map(fn($p) => [
            'id' => $p->promo_id,
            'name' => $p->promo_name,
                        ...
                        <div class="premium-card bg-white dark:bg-slate-800 border-none p-8 min-h-[600px]">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                                ...
                                <div>
                                    ...
                                    <input type="datetime-local" name="sale_date" id="sale_date_input" value="{{ date('Y-m-d\TH:i') }}" min="{{ date('Y-m-d\T00:00') }}" required
                                           class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all text-sm font-bold text-slate-700 dark:text-slate-200 shadow-inner">
                            ...
                            <div id="items-container" class="space-y-3">
                                <!-- First Item Entry (rendered by JS template) -->
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function buildRow(opts = {}) {
            // opts: { promoId, promoName, productId, productName, volume, price, stock, qty, showRemove }
            const idx    = itemIndex++;
            const frag   = template.content.cloneNode(true);
            const row    = frag.querySelector('.sale-item-row');

            // Replace __IDX__ placeholders
            ...
            if (opts.promoId) {
                ...
                // Lock strategy link to this promo & hide dropdown containers
            ...
            if (opts.productId) {
                ...
                // Lock combobox trigger (bundle items are pre-fixed)
                if (opts.promoId) {
        ...
        function attachRowListeners(row) {
            ...
            if (removeBtn) {
                removeBtn.addEventListener('click', () => { row.remove(); reindexRows(); calculateTotal(); });
            }
            ...
            // Strategy Link change → bundle expansion
            if (promoSelect) {
                promoSelect.addEventListener('change', function () {
                    const promoId = this.value;
                    if (promoId) {
                        expandBundle(promoId, row);
                    }
                });
            }
        }
        ...
        function reindexRows() {
            container.querySelectorAll('.sale-item-row').forEach((row, i) => {
        ...
        async function expandBundle(promoId, triggerRow) {
            const promoName = triggerRow?.querySelector('.promo-select')?.selectedOptions[0]?.text || '';
            try {
                ...
                if (!data.items || data.items.length === 0) {
                    ...
                    if (triggerRow) triggerRow.querySelector('.promo-select').value = '';
                    return;
                }

                // Remove trigger row (we'll replace it with bundle rows)
                ...
                Swal.fire({
                    icon: 'success',
                    title: `Bundle Added`,
                    text: `${data.items.length} item(s) from "${promoName}" added to the transaction.`,
                    timer: 2000,
