    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h2 class="font-bold text-2xl text-slate-800 dark:text-white leading-tight flex items-center gap-3">
                ...
                {{ __('Salesmen Finance Report') }}
            </h2>
            <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                <span>Salesmen:</span>
                <span class="font-bold text-slate-800 dark:text-white">{{ $salesmen->name }} ({{ $salesmen->username }})</span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="premium-card bg-white dark:bg-slate-800 p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <div class="flex flex-col lg:flex-row justify-between items-stretch lg:items-center mb-8 gap-4 pb-6 border-b border-slate-100 dark:border-slate-700">
                    ...
                    <!-- Filter Form -->
                    <form id="filterForm" action="{{ route('reports.salesmen', ['id' => $salesmen->salesmen_id]) }}" method="GET" class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="flex items-end gap-3 self-end lg:self-center">
                        <a href="{{ route('reports.salesmen.export', ['id' => $salesmen->salesmen_id, 'format' => 'excel', 'start_date' => $startDate, 'end_date' => $endDate, 'promo_id' => $promoId]) }}" 
                        ...
                        <a href="{{ route('reports.salesmen.export', ['id' => $salesmen->salesmen_id, 'format' => 'pdf', 'start_date' => $startDate, 'end_date' => $endDate, 'promo_id' => $promoId]) }}" 
                ...
                function runFilter(e) {
                    ...
                    .then(data => {
                        ...
                        const pagContainer = document.getElementById('paginationLinksContainer');
                        if (pagContainer) {
            ...
            function approveSelectedSales() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        fetch("{{ route('reports.salesmen.approve-selected', ['id' => $salesmen->salesmen_id]) }}", {
                            method: 'POST',
                            headers: {
